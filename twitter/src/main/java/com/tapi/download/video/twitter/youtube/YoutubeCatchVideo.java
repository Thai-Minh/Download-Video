package com.tapi.download.video.twitter.youtube;
...
public class YoutubeCatchVideo implements ICatch {

    private static final String TAG = "YoutubeCatchVideo";
    private static YoutubeCatchVideo instance;
    private boolean isWaitingForGettingVideo = false;

    public static YoutubeCatchVideo getInstance() {
        if (instance == null)
            instance = new YoutubeCatchVideo();
        return instance;
    }

    @Override
    public void getVideoDownloadLink(String viewLink, OnCatchVideoListener onCatchVideoListener) {
        new getLinkVideo(onCatchVideoListener).execute(viewLink);
    }
...
//        });
    ...
    private class getLinkVideo extends AsyncTask<String, Void, Video> {
        private OnCatchVideoListener listener;
        private String link;

        public getLinkVideo(OnCatchVideoListener listener) {
